...
class TrafficSignEncoder(nn.Module):
    ...
    def forward(self, now_info, history_info, map_info):
        ...
        # Scale adjustment (例子)
        # x_history = F.normalize(x_history, p=2, dim=1)  # L2 归一化
        # x_map = F.normalize(x_map, p=2, dim=1)  # L2 归一化

        # 1. 直接平均
        # output = (x_history + x_map) / 2

        # 或者 2. 使用 learnable 权重进行加权平均
        alpha = torch.sigmoid(self.alpha_linear(torch.cat((x_history, x_map), dim=1)))  # 学习到的权重
        output = alpha * x_history + (1 - alpha) * x_map

        # weights = weights.squeeze()
        # # 或者 3. 使用 learnable 权重进行加权平均
        # alpha = torch.sigmoid(self.alpha_linear(weights))  # 学习到的权重
        # output = alpha * x_history + (1 - alpha) * x_map

        # output = x_history
        # output = x_history * 0.9 + x_map * 0.1
